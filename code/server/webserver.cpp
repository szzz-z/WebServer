...
WebServer::WebServer(int port, int trigMode, int timeoutMS, bool OptLinger, int sqlPort, const char *sqlUser,
  ...
  /// 云服务器可以改成localhost 访问    但是本地只能127.0.0.1 不知道为啥
...
void WebServer::AddClient(int fd, sockaddr_in addr) {
  ...
  if (timeout_ms_ > 0) {
    timer_->Add(fd, timeout_ms_, [this, capture0 = &users_[fd]] { CloseConn(capture0); });
  }
...
auto WebServer::InitSocket() -> bool {
  ...
  ret = setsockopt(listen_fd_, SOL_SOCKET, SO_REUSEADDR, static_cast<const void *>(&optval), sizeof(int));
